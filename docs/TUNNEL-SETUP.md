...
4. Điền:
  ...
  - **Subdomain:** vd. `fnb` → URL public sẽ là `https://fnb.123zo.uk`.
...
2. Thêm:
  ...
  - **Value:** URL public tunnel, **không** có `/` cuối (vd. `https://fnb.123zo.uk`).
...
4. F12 → **Network**: request tới `/api/data/products`, `/api/data/sales`, `/api/data/imports`, … phải có **Request URL** = `https://fnb.123zo.uk/api/...` và **Status 200**. Thanh địa chỉ trình duyệt vẫn là `tbc-fnb.vercel.app`.
...
**Cách sửa:** Trong **Published application routes** → Edit route của `fnb.123zo.uk` → đổi **Service** thành `http://<IP-máy-chạy-app>:3000` (vd. `http://192.168.10.201:3000`). Lưu, đợi vài giây rồi thử lại.
...
| `tbc-fnb.vercel.app/admin`| `https://fnb.123zo.uk/api/...` | Qua tunnel khi tunnel reachable |
